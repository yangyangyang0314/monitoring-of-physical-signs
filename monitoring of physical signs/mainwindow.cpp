...
{
    ui->setupUi(this);
    resize(1180,590);//设置窗口大小
    QPixmap bgImage(":/image/background1.jpg");

    QPalette palette;
    palette.setBrush(this->backgroundRole(), QBrush(bgImage.scaled(this->size(), Qt::IgnoreAspectRatio)));
    this->setPalette(palette);

    // 设置pushButton_start按钮的背景图片
    QString imagePath1 = ":/image/button1.jpeg";
    QPixmap buttonImage1(imagePath1);
    ui->pushButton_start->setStyleSheet("QPushButton#pushButton_start { border-image: url(" + imagePath1 + "); }");

    QString imagePath2 = ":/image/stop.jpeg";
    QPixmap buttonImage2(imagePath2);
    ui->pushButton_pause->setStyleSheet("QPushButton#pushButton_pause { border-image: url(" + imagePath2 + "); }");

    QString imagePath3 = ":/image/over.jpeg";
    QPixmap buttonImage3(imagePath3);
    ui->pushButton_stop->setStyleSheet("QPushButton#pushButton_stop { border-image: url(" + imagePath3 + "); }");
 ...
 if(AUTO_DETECT_COM_PORTS)
   {
    ...
    ui->phaseWfmPlot->xAxis->setLabel("时间");
    ui->phaseWfmPlot->xAxis->setLabelColor(Qt::white);
    ui->phaseWfmPlot->xAxis->setLabelFont(font);
    ui->phaseWfmPlot->xAxis->setTickLabelColor(Qt::white);

    QPen axisPen(Qt::white);
    ui->phaseWfmPlot->xAxis->setBasePen(axisPen);
    ui->phaseWfmPlot->yAxis->setBasePen(axisPen);
    ui->phaseWfmPlot->xAxis->setTickPen(QPen(QColor(255, 255, 255)));
    ui->phaseWfmPlot->yAxis->setTickPen(QPen(QColor(255, 255, 255)));
    ui->phaseWfmPlot->yAxis->setLabel("幅度");
    ui->phaseWfmPlot->yAxis->setLabelColor(Qt::white);
    ui->phaseWfmPlot->yAxis->setLabelFont(font);
    ui->phaseWfmPlot->yAxis->setTickLabelColor(Qt::white);
    ...
    QCPTextElement *myTitle_ChestDisp = new QCPTextElement(ui->phaseWfmPlot, "胸腔时域图");
    myTitle_ChestDisp->setFont(titleFont);
    myTitle_ChestDisp->setSelectedTextColor(Qt::white);
    myTitle_ChestDisp->setTextColor(Qt::white);
    ...
    ui->BreathingWfmPlot->xAxis->setBasePen(axisPen);
    ui->BreathingWfmPlot->yAxis->setBasePen(axisPen);
    ui->BreathingWfmPlot->xAxis->setTickPen(QPen(QColor(255, 255, 255)));
    ui->BreathingWfmPlot->yAxis->setTickPen(QPen(QColor(255, 255, 255)));
    ui->BreathingWfmPlot->yAxis->setLabel("幅度");
    ui->BreathingWfmPlot->xAxis->setLabel("时间");
    ui->BreathingWfmPlot->xAxis->setLabelColor(Qt::white);
    ui->BreathingWfmPlot->xAxis->setTickLabelColor(Qt::white);
    ui->BreathingWfmPlot->yAxis->setTickLabelColor(Qt::white);
    ui->BreathingWfmPlot->yAxis->setLabelColor(Qt::white);
    ...
    QCPTextElement *myTitle_BreathWfm = new QCPTextElement(ui->BreathingWfmPlot, "呼吸波形");
    myTitle_BreathWfm->setFont(titleFont);
    myTitle_BreathWfm->setTextColor(Qt::white);
    ...
    ui->heartWfmPlot->xAxis->setLabel("时间");
    ui->heartWfmPlot->yAxis->setLabel("幅度");
    ui->heartWfmPlot->xAxis->setBasePen(axisPen);
    ui->heartWfmPlot->yAxis->setBasePen(axisPen);
    ui->heartWfmPlot->xAxis->setTickPen(QPen(QColor(255, 255, 255)));
    ui->heartWfmPlot->yAxis->setTickPen(QPen(QColor(255, 255, 255)));
    ui->heartWfmPlot->xAxis->setTickLabelColor(Qt::white);
    ui->heartWfmPlot->yAxis->setLabelColor(Qt::white);
    ui->heartWfmPlot->xAxis->setLabelColor(Qt::white);
    ui->heartWfmPlot->yAxis->setTickLabelColor(Qt::white);
    ...
    QCPTextElement *myTitle_HeartWfm = new QCPTextElement(ui->heartWfmPlot, "心率波形");
    myTitle_HeartWfm->setTextColor(Qt::white);
...
void MainWindow::on_pushButton_start_clicked()
{
   ...
   QString filenameText = "../profiles/xwr1642_profile_VitalSigns_20fps_Front.cfg";

    if (ui->radioButton_BackMeasurements->isChecked())
    {

        filenameText = "../profiles/xwr1642_profile_VitalSigns_20fps_Back.cfg";
...
}
{
 ...
 if (ui->checkBox_displayPlots->isChecked())
 {
  ...
  if (min > -BREATHING_PLOT_MAX_YAXIS)
   ...
   QString myString_HRV;
   ui->lcdNumber_HRV->setDigitCount(4);
   myString_HRV.sprintf("%1.0f",60/heartRate_Out*1000);
   ui->lcdNumber_HRV->display(myString_HRV);


   // 声明并初始化变量来保存旧的 HRV 和 SDNN 值
   static QString myString_HRV_old;
   static double SDNN_old = 0.0;
   double SDNN = 0.0;

   // 初始化旧的 HRV 值
   if (myString_HRV_old.isEmpty())
      myString_HRV_old = "0";

   // 计算新的 HRV 值
   QString myString_HRV_new;
   ui->lcdNumber_HRV->setDigitCount(4);
   myString_HRV_new.sprintf("%1.0f", 60/heartRate_Out*1000);
   ui->lcdNumber_HRV->display(myString_HRV_new);

   // 计算新旧 HRV 值的绝对差值，并赋值给 SDNN
   SDNN = sqrt(fabs(myString_HRV_new.toDouble() - myString_HRV_old.toDouble()));

   // 如果 SDNN 的值小于20，则 SDNN 的值为 20 到 40 的随机数
   //if(SDNN < 20) {
       //SDNN = (qrand() % 21) + 20;
   //}

   // 如果 SDNN 的值为零，就保持显示前一个非零的 SDNN 值；否则更新显示的 SDNN 值并更新 SDNN_old 的值
   if(SDNN == 0)
       ui->lcdNumber_sdnn->display(SDNN_old);
   else{
       ui->lcdNumber_sdnn->display(SDNN);
       SDNN_old = SDNN;
   }

   // 更新旧的 HRV 值
   myString_HRV_old = myString_HRV_new;
